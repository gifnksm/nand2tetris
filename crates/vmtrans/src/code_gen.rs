...
impl<'a> CodeGen<'a> {
    ...
    pub(crate) fn bootstrap(&mut self, name: &Ident) {
        self.load_imm_d(Imm::try_new(256).unwrap());
        self.store_d_address(Label::SP);
        self.call(name, 0);
    }
    ...
    pub(crate) fn label(&mut self, label: &Ident) {
        let label = self.make_label(label);
        self.stmts.push(S::label(label));
    }

    pub(crate) fn goto(&mut self, label: &Ident) {
        let label = self.make_label(label);
        self.jump(label);
    }

    pub(crate) fn if_goto(&mut self, label: &Ident) {
        let label = self.make_label(label);
        self.pop_d();
        self.if_jump(label, Comp::D, Jump::Ne);
    }

    pub(crate) fn function(&mut self, name: &Ident, arity: u8) {
        let label = self.make_function_label(name);
        self.stmts.push(S::label(label));
        self.push_zeros(arity);
    }

    pub(crate) fn call(&mut self, name: &Ident, arity: u8) {
        ...
        self.push_d();
        // push RAM[LCL]
        self.load_address_d(Label::LCL);
        self.push_d();
        // push RAM[ARG]
        self.load_address_d(Label::ARG);
        self.push_d();
        // push RAM[THIS]
        self.load_address_d(Label::THIS);
        self.push_d();
        // push RAM[THAT]
        self.load_address_d(Label::THAT);
        self.push_d();
        // RAM[ARG] = RAM[SP] - n - 5
        self.load_address_d(Label::SP);
        self.stmts.extend([
            S::a(Imm::from(arity + 5)),
            S::c(Dest::D, Comp::DMinusA, Jump::Null),
        ]);
        self.store_d_address(Label::ARG);
        // LCL = SP
        self.load_address_d(Label::SP);
        self.store_d_address(Label::LCL);
        // goto f
    ...
    pub(crate) fn return_(&mut self) {
        fn set(stmts: &mut Vec<Statement>, dest: Label, base: Label, n: u8) {
        ...
        // RAM[R13] = RAM[LCL]
        self.load_address_d(Label::LCL);
        self.store_d_address(Label::R13);
        // RAM[R14] = RAM[RAM[R13] - 5]
        set(&mut self.stmts, Label::R14, Label::R13, 5);
        // RAM[RAM[ARG]] = pop()
        self.pop_d();
        self.stmts.extend([
            S::at_label(Label::ARG),
        ...
        set(&mut self.stmts, Label::THAT, Label::R13, 1);
        // RAM[THIS] = RAM[RAM[R13] - 2]
        set(&mut self.stmts, Label::THIS, Label::R13, 2);
        // RAM[ARG] = RAM[RAM[R13] - 3]
        set(&mut self.stmts, Label::ARG, Label::R13, 3);
        // RAM[LCL] = RAM[RAM[R13] - 4]
        set(&mut self.stmts, Label::LCL, Label::R13, 4);
        // goto RAM[R14]
        self.stmts.extend([
    ...
    fn make_static_label(&self, index: Imm) -> Label {
    ...
    fn make_function_label(&self, name: &Ident) -> Label {
        Label::from(name.as_str().to_string())
    }

    fn set_segment_addr_to_d(&mut self, base_register: Label, index: Imm) {
        self.stmts.extend([
    ...
    fn set_segment_addr_to_a(&mut self, base_register: Label, index: Imm) {
        self.stmts.extend([
            S::at_label(base_register),
            S::c(Dest::D, Comp::M, Jump::Null),
            S::a(index),
    ...
    fn load_address_d(&mut self, label: Label) {
        self.stmts.extend([
            // D = RAM[label]
            S::at_label(label),
            S::c(Dest::D, Comp::M, Jump::Null),
    ...
    fn load_dynamic_segment_d(&mut self, base_register: Label, index: Imm) {
    ...
    fn store_d_dynamic_segment(&mut self, base_register: Label, index: Imm) {
        self.stmts.extend([
            // RAM[R13] = D
    ...
    fn store_d_address(&mut self, label: Label) {
        self.stmts.extend([
            // RAM[label] = D
            S::at_label(label),
            S::c(Dest::M, Comp::D, Jump::Null),
    ...
    fn push_zeros(&mut self, count: u8) {
        self.stmts.push(S::at_label(Label::SP));
        for _ in 0..count {
            self.stmts.extend([
                // RAM[RAM[SP]] = 0
                S::c(Dest::A, Comp::M, Jump::Null),
    ...
    fn if_jump(&mut self, label: Label, comp: Comp, jump: Jump) {
        self.stmts.extend([
            // if (D) jump label
            S::at_label(label),
